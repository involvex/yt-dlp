...
class LocipoIE(LocipoBaseIE):
    _VALID_URL = [
        rf"https?://locipo\.jp/creative/(?P<id>{LocipoBaseIE._UUID_RE})",
        rf"https?://locipo\.jp/embed/?\?(?:[^#]+&)?id=(?P<id>{LocipoBaseIE._UUID_RE})",
    ]
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        media_id = traverse_obj(
            creatives, ("media_id", {str}, {require("Streaks media ID")})
        )

        webpage = self._download_webpage(url, video_id)
        config = self._search_json(
            r"window\.__NUXT__\.config\s*=",
            webpage,
            "config",
            video_id,
            transform_source=js_to_json,
        )
        api_key = traverse_obj(
            config, ("public", "streaksVodPlaybackApiKey", {str}, {require("api key")})
        )
...
class LocipoPlaylistIE(LocipoBaseIE):
    _VALID_URL = [
        rf"https?://locipo\.jp/(?P<type>playlist)/(?P<id>{LocipoBaseIE._UUID_RE})",
    ...
    def _real_extract(self, url):
        ...
        if urlh := self._request_webpage(HEADRequest(url), playlist_id, fatal=False):
            playlist_type, playlist_id = self._match_valid_url(urlh.url).group(
                "type", "id"
            )
        ...
        entries = InAdvancePagedList(
            ...
            math.ceil(int(creatives["total"]) / self._PAGE_SIZE),
            self._PAGE_SIZE,
        )
