...
class BandcampWeeklyIE(BandcampIE):  # XXX: Do not subclass from concrete IE
    ...
    _VALID_URL = (
        r"https?://(?:www\.)?bandcamp\.com/radio/?\?(?:[^#]+&)?show=(?P<id>\d+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        return {
            ...
            "title": join_nonempty(
                series_title,
                strftime_or_none(release_timestamp, "%Y-%m-%d"),
                delim=", ",
            ),
            "series": series_title,
            "thumbnail": format_field(
                show_data, "imageId", "https://f4.bcbits.com/img/%s_0.jpg", default=None
            ),
