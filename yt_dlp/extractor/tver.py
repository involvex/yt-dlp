...
class TVerOlympicIE(StreaksBaseIE):
    ...
    def _real_extract(self, url):
        ...
        if video_type == "live":
            ...
            olympic_data = traverse_obj(
                self._download_json(f"{self._API_BASE}/live/{video_id}", video_id),
                ("contents", "live", {dict}),
            )
            media_id = traverse_obj(olympic_data, ("video_id", {str}))

            now = time_seconds()
            start_timestamp_str = traverse_obj(
                olympic_data, ("onair_start_date", {str})
            )
            ...
            end_timestamp = traverse_obj(
                olympic_data,
                (
                    "onair_end_date",
                    {unified_timestamp(tz_offset=9)},
                    {require("on-air end time")},
                ),
            )

            if now < start_timestamp:
                self.raise_no_formats(
                    f"This program is scheduled to start at {start_timestamp_str} JST",
                    expected=True,
                )
                ...
                dvr_end_timestamp = traverse_obj(
                    olympic_data, ("dvr_end_date", {unified_timestamp(tz_offset=9)})
                )
            ...
            live_status = "not_live"
            olympic_data = traverse_obj(
                self._download_json(f"{self._API_BASE}/video/{video_id}", video_id),
                ("contents", "video", {dict}),
            )
        ...
        return {
            ...
            **traverse_obj(
                olympic_data,
                {
                    ...
                    "description": (
                        ("description", "description_l", "description_s"),
                        {clean_html},
                        filter,
                        any,
                    ),
                    ...
                    "thumbnail": (
                        ("picture_l_url", "picture_m_url", "picture_s_url"),
                        {url_or_none},
                        any,
                    ),
                },
