    ...
    def _real_extract(self, url):
        ...
        metadata = {
            ...
            for m in re.finditer(
                r"(?:^|\|)(?P<k>[a-z_]+)=(?P<v>(?:(?!\|[a-z_]+=).)+)", attrs
            )
        }
        m3u8_url = urllib.parse.unquote(
            self.decrypt(encrypted_data, metadata["key"])
        ).rstrip("\x0e\x0f")
...
class TarangPlusEpisodesIE(TarangPlusBaseIE):
    ...
    def _entries(self, playlist_url, playlist_id, page):
        data = self._download_json(
            playlist_url,
            playlist_id,
            f"Downloading playlist JSON page {page + 1}",
            query={"page_no": page},
        )
    ...
    def _entries(self, webpage):
        for url_path in traverse_obj(
            ...
            (
                ...
                {
                    find_elements(
                        tag="a", attr="href", value="/.+", html=True, regex=True
                    )
                },
                ...,
                {extract_attributes},
