...
class VolejTVIE(VolejTVBaseIE):
    ...
    def _real_extract(self, url):
        ...
        for video in traverse_obj(
            json_data,
            ("videos", 0, "qualities", lambda _, v: url_or_none(v["cloud_front_path"])),
        ):
            formats.append(
                traverse_obj(
                    video,
...
class VolejTVPlaylistBaseIE(VolejTVBaseIE):
    ...
    def _get_page(self, playlist_id, page):
        return self._call_api(
            f"match/{self._API_FILTER}/{playlist_id}",
            playlist_id,
...
class VolejTVClubPlaylistIE(VolejTVPlaylistBaseIE):
    ...
    def _real_extract(self, url):
        ...
        total_pages = traverse_obj(
            first_page_data, ("meta", "pageCount", {int}, {require("page count")})
        )
...
class VolejTVCategoryPlaylistIE(VolejTVPlaylistBaseIE):
    ...
    def _get_category(self, playlist_id):
        ...
        for category in traverse_obj(
            categories, (lambda _, v: v["slug"] and v["id"] and v["title"])
        ):
