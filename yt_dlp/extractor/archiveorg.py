...
class ArchiveOrgIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        metadata = self._download_json(
            f"https://archive.org/metadata/{identifier}", identifier
        )
        m = metadata["metadata"]
...
class YoutubeWebArchiveIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if video_info.get("dmux"):
            for vf in traverse_obj(
                video_info, ("formats", "video", lambda _, v: url_or_none(v["url"]))
            ):
                formats.append(self._parse_fmt(vf, {"acodec": "none"}))
            for af in traverse_obj(
                video_info, ("formats", "audio", lambda _, v: url_or_none(v["url"]))
            ):
