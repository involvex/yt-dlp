...
class TheChosenIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        metadata = self._download_json(
            f"https://api.watch.thechosen.tv/v1/videos/{video_id}", video_id
        )

        formats, subtitles = [], {}
        for fmt_url in traverse_obj(
            metadata, ("details", "video", ..., "url", {url_or_none})
        ):
            ...
            if ext == "m3u8":
                ...
                fmts, subs = self._extract_mpd_formats_and_subtitles(
                    fmt_url, video_id, fatal=False
                )
            else:
                self.report_warning(
                    f'Skipping unsupported format extension "{ext}"', video_id=video_id
                )
                continue
            formats.extend(fmts)
        ...
        thumbnails = []
        for thumb_id, thumb_url in traverse_obj(
            metadata,
            (
                ("thumbs", "thumbnails"),
                {dict.items},
