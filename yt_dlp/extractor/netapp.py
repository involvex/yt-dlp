...
class NetAppVideoIE(NetAppBaseIE):
    ...
    def _real_extract(self, url):
        ...
        brightcove_video_id = traverse_obj(
            metadata,
            (
                "sections",
                ...
                {str},
                any,
                {require("brightcove video id")},
            ),
        )

        video_item = traverse_obj(
            metadata, ("sections", lambda _, v: v["type"] == "VideoDetail", any)
        )

        return self.url_result(
            self._BC_URL.format(brightcove_video_id),
            BrightcoveNewIE,
            brightcove_video_id,
            url_transparent=True,
            **self._parse_metadata(video_item),
        )
...
class NetAppCollectionIE(NetAppBaseIE):
    ...
    def _entries(self, metadata):
        for item in traverse_obj(
            metadata, ("items", lambda _, v: v["brightcoveVideoId"])
        ):
    ...
    def _real_extract(self, url):
        ...
        metadata = self._download_json(
            f"https://api.media.netapp.com/client/collection/{collection_uuid}",
            collection_uuid,
        )

        return self.playlist_result(
            self._entries(metadata),
            collection_uuid,
