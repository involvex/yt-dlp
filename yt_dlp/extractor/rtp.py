...
class RTPIE(InfoExtractor):
    ...
    _TESTS = [
        ...
        },
        {
            ...
            "expected_warnings": [
                "Episode data not found in API response; falling back to web extraction"
            ],
        },
        {
    ...
    def _extract_asset(
        self, asset_data, episode_id, episode_info, archive_compat=False
    ):
        ...
        return {
            **episode_info,
            ...
            "_old_archive_ids": (
                [make_archive_id(self, episode_id)] if archive_compat else None
            ),
            "formats": formats,
            "subtitles": subtitles,
            **traverse_obj(
    ...
    def _report_fallback_warning(
        self, missing_info_name="required info", display_id=None
    ):
        self.report_warning(
    ...
    def _entries(self, assets, episode_id, episode_info):
        ...
        for idx, asset_data in enumerate(assets):
            yield self._extract_asset(
                asset_data, episode_id, episode_info, archive_compat=not idx
            )
    ...
    def _extract_from_api(self, program_id, episode_id, asset_id):
        ...
        if asset_id:
            asset_data = traverse_obj(
                assets, (lambda _, v: v["asset_id"] == asset_id, any)
            )
            if not asset_data:
        ...
        if self._yes_playlist(
            len(assets) > 1 and episode_id,
            asset_data["asset_id"],
            playlist_label="multi-part episode",
            video_label="individual part",
        ):
        ...
        return self._extract_asset(
            asset_data, episode_id, episode_info, archive_compat=True
        )
    ...
    def _extract_from_html(self, url, program_id, episode_id, asset_id):
        ...
        if not asset_id:
            asset_id = self._search_regex(
                r'\basset_id\s*:\s*"(\d+)"', webpage, "asset ID"
            )
    ...
    def _real_extract(self, url):
        program_id, episode_id, asset_id = self._match_valid_url(url).group(
            "program_id", "episode_id", "asset_id"
        )
