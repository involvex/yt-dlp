...
class YoutubeIE(YoutubeBaseInfoExtractor):
    ...
    def _player_js_version(self):
        return (
            self._configuration_arg("player_js_version", [None])[0]
            or self._DEFAULT_PLAYER_JS_VERSION
        )

    @functools.cached_property
    def _webpage_client(self):
        webpage_client = self._configuration_arg(
            ...
            for retry in self.RetryManager(fatal=False):
                ...
                f = next(
                    (
                        f
                        for f in formats
                        if f.get("_itag") == itag and f.get("_client") == client_name
                    ),
                    None,
                )
                if not f:
    ...
    def _live_dash_fragments(
        ...
        def _extract_sequence_from_mpd(refresh_sequence, immediate):
            ...
            mpd_url, stream_number, is_live = mpd_feed(
                itag, client_name, 5 if expire_fast else 18000
    ...
    def _load_player_data_from_cache(
        self, name, player_url, *cache_keys, use_disk_cache=False
    ):
        cache_id = (
            f"youtube-{name}",
            self._player_js_cache_key(player_url),
            *map(str_or_none, cache_keys),
        )
        ...
        data = self.cache.load(
            cache_id[0], join_nonempty(*cache_id[1:]), min_ver="2025.07.21"
        )
        if data:
    ...
    def _store_player_data_to_cache(
        self, data, name, player_url, *cache_keys, use_disk_cache=False
    ):
        cache_id = (
            f"youtube-{name}",
            self._player_js_cache_key(player_url),
    ...
    def _comment_entries(
        ...
        def extract_thread(contents, entity_payloads, thread_parent, thread_depth):
            ...
            for content in contents:
                ...
                tracker[
                    "total_reply_comments" if thread_parent else "total_parent_comments"
                ] += 1
                yield comment
                ...
                if comment_replies_renderer:
                    subthreads = traverse_obj(
                        comment_replies_renderer, ("subThreads", ..., {dict})
                    )
                    # Recursively extract from `commentThreadRenderer`s in `subThreads`
                    if threads := traverse_obj(
                        subthreads, lambda _, v: v["commentThreadRenderer"]
                    ):
                        for entry in extract_thread(
                            threads, entity_payloads, comment_id, thread_depth + 1
                        ):
                            if entry:
                                yield entry
                        if not traverse_obj(
                            subthreads, lambda _, v: v["continuationItemRenderer"]
                        ):
        ...
        (
            ...
            *_,
            ...
            for p in self._configuration_arg("max_comments") + [""] * 5
        )
        ...
        for page_num in itertools.count(0):
            ...
            for continuation_items in traverse_obj(
                ...
                for entry in extract_thread(
                    continuation_items, mutations, parent, depth
                ):
                    if not entry:
                        return
                    yield entry
    ...
    @classmethod
    def _generate_player_context(
        cls, sts=None, use_ad_playback_context=False, encrypted_context=None
    ):
        context = {
        ...
        **kwargs,
        ...
        if "true" in traverse_obj(
            ...
            self.write_debug(
                ...
                f"to video ID for {client} client",
                only_once=True,
            )
    ...
    def _extract_player_response(
        ...
        use_ad_playback_context = self._configuration_arg(
            "use_ad_playback_context", ["false"]
        )[0] != "false" and traverse_obj(
            INNERTUBE_CLIENTS, (client, "SUPPORTS_AD_PLAYBACK_CONTEXT", {bool})
        )
    ...
    def _extract_player_responses(
        ...
        while clients:
            ...
            if pr_id := self._invalid_player_response(pr, video_id):
                ...
                sd[STREAMING_DATA_AVAILABLE_AT_TIMESTAMP] = (
                    self._get_available_at_timestamp(pr, video_id, client)
                )
    ...
    def _extract_formats_and_subtitles(
                ...
                challenge_requests.append(
                    JsChallengeRequest(
                        ...
                        input=NChallengeInput(
                            challenges=list(n_challenges), player_url=player_url
                        ),
                    )
                )
            if s_challenges:
                ...
                for spec_id in s_challenges:
                    if self._load_player_data_from_cache(
                        "sigfuncs", player_url, spec_id, use_disk_cache=True
                    ):
        ...
        for streaming_data in traverse_obj(
            player_responses, (..., "streamingData", {dict})
        ):
            # HTTPS formats
            for fmt_stream in traverse_obj(
                streaming_data, (("formats", "adaptiveFormats"), ..., {dict})
            ):
        ...
        for pr in player_responses:
            ...
            def process_https_formats():
                ...
                for fmt_stream in streaming_formats:
                    ...
                    if not fmt_url:
                        ...
                        if not all(
                            (sc, fmt_url, skip_player_js or player_url, encrypted_sig)
                        ):
                            msg_tmpl = (
                            ...
                            if client_name in ("web", "web_safari"):
                                ...
                                self.report_warning(
                                    msg_tmpl.format("", client_name, msg),
                                    video_id,
                                    only_once=True,
                                )
                            continue
                    ...
                    if query.get("n"):
                        ...
                        solve_js_challenges()
                        n_result = self._load_player_data_from_cache(
                            "n", player_url, n_challenge
                        )
                        if not n_result:
                            continue
            ...
            if hls_manifest_url:
                ...
                if m := re.fullmatch(
                    r"(?P<path>.+)(?P<suffix>/(?:file|playlist)/index\.m3u8)",
                    manifest_path,
                ):
                ...
                if (
                    ...
                    hls_manifest_url = update_url(
                        hls_manifest_url, path=f"{manifest_path}{manifest_suffix}"
                    )
            ...
            if dash_manifest_url:
                ...
                if (
                    ...
                    dash_manifest_url = update_url(
                        dash_manifest_url, path=manifest_path
                    )
    ...
    def _get_available_at_timestamp(self, player_response, video_id, client):
        ...
        for renderer in traverse_obj(
            ...
            (
                "adSlots",
                lambda _, v: v["adSlotRenderer"]["adSlotMetadata"]["triggerEvent"]
            ...
            skip_time = traverse_obj(
                renderer, ("skipOffsetMilliseconds", {float_or_none(scale=1000)})
            )
    ...
    def _real_extract(self, url):
        ...
        def adjust_incomplete_format(
            fmt, note_suffix="(Last 2 hours)", pref_adjustment=-10
        ):
            fmt["preference"] = (fmt.get("preference") or -1) + pref_adjustment
            fmt["format_note"] = join_nonempty(
                fmt.get("format_note"), note_suffix, delim=" "
            )
        ...
        if live_status in ("is_live", "post_live"):
            for fmt in formats:
                ...
                if live_status == "post_live" and is_adaptive:
                    ...
                    adjust_incomplete_format(
                        fmt, note_suffix="(ended)", pref_adjustment=-5000
                    )
                # Is it live with --live-from-start? Or is it post-live and its duration is >2hrs?
                    ...
                    if protocol == "http_dash_segments":
                        ...
                        adjust_incomplete_format(
                            fmt, note_suffix="(incomplete)", pref_adjustment=-20
                        )
