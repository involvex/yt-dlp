...
class SoundcloudBaseIE(InfoExtractor):
    ...
    def _update_client_id(self):
        webpage = self._download_webpage(
            "https://soundcloud.com/", None, "Downloading main page"
        )
        for src in reversed(re.findall(r'<script[^>]+src="([^"]+)"', webpage)):
            script = self._download_webpage(
                src, None, "Downloading JS asset", fatal=False
            )
            if script:
...
class SoundcloudPagedPlaylistBaseIE(SoundcloudBaseIE):
    ...
    def _browser_impersonate_target(self):
        ...
        if not filtered or version_as_float(filtered[-1]) < version_as_float(
            MIN_SUPPORTED_TARGET
        ):
