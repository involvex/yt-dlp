...
class ZapiksIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if embed_url := traverse_obj(
            webpage,
            (
                {find_element(cls="embed-container")},
        ...
        data_media_url = traverse_obj(
            video_responsive, ("data-media-url", {url_or_none})
        )
        if data_media_url and urllib.parse.urlparse(url).path == "/index.php":
            return self.url_result(data_media_url, ZapiksIE)

        data_playlist = traverse_obj(
            video_responsive, ("data-playlist", {json.loads}, ..., any)
        )
        formats = []
        ...
        return {
            ...
            **traverse_obj(
                ...
                {
                    "description": (
                        {find_element(cls="description-text")},
                        {clean_html},
                        filter,
                    ),
                    "tags": (
                ...
                (
                    {find_element(cls="video-content-user-link", html=True)},
                    {
                        ...
                        "uploader_id": (
                            {extract_attributes},
                            "href",
                            {self._UPLOADER_ID_RE.fullmatch},
                            "id",
                        ),
                    },
                ),
