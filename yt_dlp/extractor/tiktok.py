...
class TikTokBaseIE(InfoExtractor):
    ...
    def _solve_challenge_and_set_cookies(self, webpage):
        challenge_data = traverse_obj(
            webpage,
            (
                {find_element(id="cs", html=True)},
                {extract_attributes},
                "class",
                filter,
                {lambda x: f"{x}==="},
        ...
        wci_cookie_name = traverse_obj(
            webpage,
            (
                {find_element(id="wci", html=True)},
                {extract_attributes},
                "class",
                {require("challenge cookie name")},
            ),
        )

        # At time of writing, the **optional** rci cookie name was `waforiginalreid`
        rci_cookie_name = traverse_obj(
            webpage,
            ({find_element(id="rci", html=True)}, {extract_attributes}, "class"),
        )
        rci_cookie_value = traverse_obj(
            webpage, ({find_element(id="rs", html=True)}, {extract_attributes}, "class")
        )

        # Actual JS sets Max-Age=1 for the cookies, but we'll manually clear them later instead
        expire_time = (
            int(time.time()) + (self.get_param("sleep_interval_requests") or 0) + 120
        )
        self._set_cookie(
            ".tiktok.com", wci_cookie_name, wci_cookie_value, expire_time=expire_time
        )
    ...
    def _extract_web_data_and_status(self, url, video_id, fatal=True):
        ...
        def get_webpage(note="Downloading webpage"):
            res = self._download_webpage_handle(
                url, video_id, note, fatal=fatal, impersonate=True
            )
            if res is False:
                return False
        ...
        status = (
            traverse_obj(universal_data, ("webapp.video-detail", "statusCode", {int}))
            or 0
        )
