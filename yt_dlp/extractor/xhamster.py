...
class _ByteGenerator:
    ...
    def _algo6(self, s):
        ...
        s = self._s = int_to_int32(
            s * int_to_int32(0x2C9277B5) + int_to_int32(0xAC564B05)
        )
...
class XHamsterIE(InfoExtractor):
    ...
    def _decipher_format_url(self, format_url, format_id):
        ...
        if re.fullmatch(self._VALID_HEX_RE, format_url):
            ...
            if re.fullmatch(r"[0-9a-fA-F]+", format_url):
                ...
                self.write_debug(
                    f'Skipping dummy ciphertext for "{format_id}": {format_url}'
                )
            else:
                # Something has likely changed on the site's end, so we need to warn
                self.report_warning(
                    f'Skipping format "{format_id}": invalid ciphertext'
                )
            return None
        ...
        hex_string, path_remainder = self._search_regex(
            rf"^/(?P<hex>{self._VALID_HEX_RE})(?P<rem>[/,].+)$",
            parsed_url.path,
            "url components",
            default=(None, None),
            group=("hex", "rem"),
        ...
        if not hex_string:
            self.report_warning(
                f'Skipping format "{format_id}": unsupported URL format'
            )
            return None
    ...
    def _real_extract(self, url):
        ...
        webpage, urlh = self._download_webpage_handle(
            desktop_url, video_id, impersonate=True
        )
