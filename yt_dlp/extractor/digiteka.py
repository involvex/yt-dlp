...
class DigitekaIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        video_info = self._download_json(
            f"https://www.ultimedia.com/player/getConf/{self._IFRAME_MD_ID}/1/{video_id}",
            video_id,
        ...
        if hls_url := traverse_obj(
            video_info, ("media_sources", "hls", "hls_auto", {url_or_none})
        ):
        ...
        for format_id, mp4_url in traverse_obj(
            video_info, ("media_sources", "mp4", {dict.items}, ...)
        ):
            if not mp4_url:
                continue
            formats.append(
