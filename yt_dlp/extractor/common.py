...
class InfoExtractor:
    ...
    def _get_netrc_login_info(self, netrc_machine=None):
        ...
        if not netrc_machine:
            raise ExtractorError(
                f"Missing netrc_machine and {type(self).__name__}._NETRC_MACHINE"
            )
        ALLOWED = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789.-_"
        if netrc_machine.startswith(("-", "_")) or not all(
            c in ALLOWED for c in netrc_machine
        ):
            raise ExtractorError(
                f"Invalid netrc machine: {netrc_machine!r}", expected=True
            )
