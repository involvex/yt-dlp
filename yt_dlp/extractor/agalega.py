...
class AGalegaBaseIE(InfoExtractor):
    ...
    def _call_api(self, endpoint, display_id, note, fatal=True, query=None):
        if not AGalegaBaseIE._access_token or self._jwt_is_expired(
            AGalegaBaseIE._access_token
        ):
            self._refresh_access_token(endpoint)
        return self._download_json(
            f"https://api-agalega.interactvty.com/api/2.0/contents/{endpoint}",
            display_id,
            note=note,
            fatal=fatal,
            query=query,
...
class AGalegaIE(AGalegaBaseIE):
    ...
    def _real_extract(self, url):
        ...
        resource_data = self._call_api(
            f"content_resources/{video_id}/",
            video_id,
            note="Downloading resource data",
            query={
        ...
        for m3u8_url in traverse_obj(
            resource_data, ("results", ..., "media_url", {url_or_none})
        ):
