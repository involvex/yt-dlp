...
class PandaTvIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        video_meta = self._download_json(
            ...
            "Unable to download video meta data",
        ...
        if error_code := traverse_obj(video_meta, ("errorData", "code", {str})):
            if error_code == "castEnd":
                ...
                self.raise_login_required(
                    "Adult verification is required for this stream"
                )
            elif error_code == "needLogin":
                self.raise_login_required("Login is required for this stream")
            elif error_code == "needCoinPurchase":
                raise ExtractorError(
                    "Coin purchase is required for this stream", expected=True
                )
            elif error_code == "needUnlimitItem":
                raise ExtractorError(
                    "Ticket purchase is required for this stream", expected=True
                )
                ...
                raise ExtractorError(
                    join_nonempty(
                        ...
                        error_code,
                        error_msg and "with error message:",
                        error_msg,
                        delim=" ",
                    )
                )
