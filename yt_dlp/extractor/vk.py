...
class VKBaseIE(InfoExtractor):
    ...
    def _download_webpage_handle(
        ...
        challenge_hash = traverse_obj(
            challenge_url, ({parse_qs}, "hash429", -1, {require("challenge hash")})
        )

        func_code = self._search_regex(
            r"(?s)var\s+salt\s*=\s*\(\s*function\s*\(\)\s*(\{.+?\})\s*\)\(\);\s*var\s+hash",
            webpage,
            "JS challenge salt function",
        )
        ...
        self._request_webpage(
            update_url_query(challenge_url, {"key": key_hash}),
            video_id,
            "Submitting JS challenge solution",
