...
class VisirIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        video_type, video_id, display_id = self._match_valid_url(url).group(
            "type", "id", "slug"
        )
        ...
        if video_type == "player":
            ...
            if (
                not self.suitable(real_url)
                or self._match_valid_url(real_url).group("type") == "player"
            ):
                raise UnsupportedError(real_url)
            return self.url_result(real_url, self.ie_key())

        upload_date = None
        date_elements = traverse_obj(
            webpage,
            (
                {find_element(cls="article-item__date")},
                {clean_html},
                filter,
                {str.split},
            ),
        ...
        return {
            ...
            **traverse_obj(
                webpage,
                (
                    {find_element(cls="article-item press-ads")},
                    {
                        "description": (
                            {find_element(cls="-large")},
                            {clean_html},
                            filter,
                        ),
                        "view_count": (
                            {find_element(cls="article-item__viewcount")},
                            {clean_html},
                            {int_or_none},
                        ),
                    },
