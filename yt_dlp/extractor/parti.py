...
class PartiBaseIE(InfoExtractor):
    def _call_api(self, path, video_id, note=None):
        return self._download_json(
            f"https://prod-api.parti.com/parti_v2/profile/{path}",
            video_id,
            note,
            headers={
    ...
    def _real_extract(self, url):
        ...
        return {
            ...
            "formats": self._extract_m3u8_formats(
                urljoin("https://media.parti.com/", data["livestream_recording"]),
                video_id,
                "mp4",
            ),
            **traverse_obj(
...
class PartiLivestreamIE(PartiBaseIE):
    ...
    def _real_extract(self, url):
        ...
        return {
            "id": creator_slug,
            "formats": self._extract_m3u8_formats(
                channel_info["playback_url"], creator_slug, live=True
            ),
            "is_live": True,
            **traverse_obj(
                data,
