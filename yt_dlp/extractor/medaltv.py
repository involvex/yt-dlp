...
class MedalTVIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        content_data = self._download_json(
            f"https://medal.tv/api/content/{video_id}",
            video_id,
        ...
        formats = [
            fmt for fmt in formats if "video/privacy-protected-guest" not in fmt["url"]
        ]
        if not formats:
            # Fallback, does not require auth
            self.report_warning(
                "Video formats are not available through API, falling back to social video URL"
            )
            urlh = self._request_webpage(
                f"https://medal.tv/api/content/{video_id}/socialVideoUrl",
                video_id,
            ...
            **traverse_obj(
                content_data,
                {
                    ...
                    "uploader_url": (
                        ...
                        {str},
                        filter,
                        {lambda x: x and f"https://medal.tv/users/{x}"},
                    ),
                    "tags": ("tags", ..., {str}),
