...
class TelecincoBaseIE(InfoExtractor):
    def _parse_content(self, content, url):
        ...
        return {
            ...
            "formats": formats,
            "thumbnail": (
                traverse_obj(content, ("dataPoster", 1, {url_or_none}))
...
class TelecincoIE(TelecincoBaseIE):
    ...
    def _real_extract(self, url):
        ...
        description = traverse_obj(
            props_list, (..., "leadParagraph", 1, {clean_html}, any, filter)
        )
        main_content = traverse_obj(
            props_list,
            (..., ("content", ("articleData", 1, "opening")), 1, {dict}, any),
        )

        if (
            traverse_obj(props_list, (..., "editorialType", 1, {str}, any)) != "VID"
        ):  # e.g. 'ART'
            entries = []

            for p in traverse_obj(
                props_list,
                (..., "articleData", 1, ("opening", ("body", 1, ...)), 1, {dict}),
            ):
                type_ = traverse_obj(p, ("type", 1, {str}))
                content = traverse_obj(
                    p, ("content", 1, {str} if type_ == "paragraph" else {dict})
                )
                if not content:
                    continue
                if type_ == "paragraph":
                    ...
                    self.report_warning(
                        f'Skipping unsupported content type "{type_}"',
                        display_id,
                        only_once=True,
                    )
            ...
            return self.playlist_result(
                entries,
                traverse_obj(props_list, (..., "id", 1, {int}, {str_or_none}, any))
                or display_id,
