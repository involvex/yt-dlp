...
class TestHTTPRequestHandler(TestRequestHandlerBase):
    ...
    def test_unicode_path_redirection(self, handler):
        with handler() as rh:
            r = validate_and_send(
                rh, Request(f"http://127.0.0.1:{self.http_port}/302-non-ascii-redirect")
            )
        ...
        with handler() as rh:
            for bad_status in (400, 500, 599, 302):
                with pytest.raises(HTTPError):
                    validate_and_send(
                        rh,
                        Request(f"http://127.0.0.1:{self.http_port}/gen_{bad_status}"),
                    )
    ...
    def test_redirect_loop(self, handler):
        with handler() as rh:
            with pytest.raises(HTTPError, match="redirect loop"):
                validate_and_send(
                    rh, Request(f"http://127.0.0.1:{self.http_port}/redirect_loop")
                )
    ...
    def test_incompleteread(self, handler):
        with handler(timeout=2) as rh:
            with pytest.raises(
                ...
                validate_and_send(
                    rh, Request(f"http://127.0.0.1:{self.http_port}/incompleteread")
                ).read()
    ...
    def test_source_address(self, handler):
        ...
        with handler(source_address=source_address) as rh:
            data = (
                validate_and_send(
                    rh, Request(f"http://127.0.0.1:{self.http_port}/source_address")
                )
                .read()
                .decode()
            )
    ...
    def test_gzip_trailing_garbage(self, handler):
        with handler() as rh:
            res = validate_and_send(
                rh, Request(f"http://localhost:{self.http_port}/trailing_garbage")
            )
    ...
    @pytest.mark.skip_handlers_if(
        lambda _, handler: Features.NO_PROXY not in handler._SUPPORTED_FEATURES,
    ...
    @pytest.mark.skip_handlers_if(
        lambda _, handler: Features.ALL_PROXY not in handler._SUPPORTED_FEATURES,
    ...
    def test_supported_impersonate_targets(self, handler):
        with handler(headers=std_headers) as rh:
            ...
            for target in rh.supported_targets:
                ...
                assert res.status == 200
                assert (
                    std_headers["user-agent"].lower() not in res.read().decode().lower()
                )
    ...
    def test_http_error_returns_content(self, handler):
        ...
        def get_response():
            with handler() as rh:
                ...
                try:
                    validate_and_send(
                        rh, Request(f"http://127.0.0.1:{self.http_port}/gen_404")
                    )
                except HTTPError as e:
                    return e.response
...
class TestRequestsRequestHandler(TestRequestHandlerBase):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                lambda: requests.exceptions.SSLError("12[CERTIFICATE_VERIFY_FAILED]34"),
                CertificateVerifyError,
            ),
...
class TestRequestHandlerValidation:
    ...
    EXTENSION_TESTS = [
        ...
        (
            ...
            [
                ...
                ({"unsupported": "value"}, UnsupportedRequest),
                (
                    {"impersonate": ImpersonateTarget("badtarget", None, None, None)},
                    UnsupportedRequest,
                ),
    ...
    def test_proxy_key(self, handler, scheme, proxy_key, proxy_scheme, fail):
        run_validation(
            ...
            fail,
            Request(
                f"{scheme}://", proxies={proxy_key: f"{proxy_scheme}://example.com"}
            ),
        )
        run_validation(
            handler,
    ...
    def test_proxy_scheme(self, handler, req_scheme, scheme, fail):
        run_validation(
            ...
            fail,
            Request(
                f"{req_scheme}://", proxies={req_scheme: f"{scheme}://example.com"}
            ),
        )
        run_validation(
            handler,
...
class TestRequestDirector:
    ...
    def test_handler_operations(self):
        ...
        assert (
            director.handlers.get(AnotherFakeRH.RH_KEY).RH_KEY == AnotherFakeRH.RH_KEY
        )
    ...
    def test_compat_request(self):
        with FakeRHYDL() as ydl:
            ...
            urllib_req = urllib.request.Request(
                "http://foo.bar", data=b"test", method="PUT", headers={"X-Test": "1"}
            )
...
class TestRequest:
    ...
    def test_content_type_header(self):
        req = Request(
            "http://example.com", headers={"Content-Type": "test"}, data=b"test"
        )
    ...
    def test_url(self):
        ...
        assert (
            req.url == "https://xn--test-z6d.example.com/%20some%20space%D0%B2?%C3%A4=c"
        )
...
class TestResponse:
    ...
    def test_reason(self, reason, status, expected):
        res = Response(
            io.BytesIO(b""), url="test://", headers={}, status=status, reason=reason
        )
    ...
    def test_compat(self):
        res = Response(
            io.BytesIO(b""), url="test://", status=404, headers={"test": "test"}
        )
        with warnings.catch_warnings():
            warnings.simplefilter("ignore", category=DeprecationWarning)
