...
class TestTraversal:
    ...
    def test_traversal_function(self):
        ...
        assert traverse_obj(_TEST_DATA, lambda _, x: isinstance(x[0], str)) == [
            "str"
    ...
    def test_traversal_branching_nesting(self):
        ...
        assert traverse_obj(_TEST_DATA, ("urls", ((1, "fail"), (0, "url")))) == [
            "https://www.example.com/0"
        ...
        assert (
            traverse_obj(_TEST_DATA, ("urls", ((0, ("fail", "url")), (1, "url"))))
            == expected
        ), "tripple nesting in path should be treated as branches"
        assert (
            traverse_obj(_TEST_DATA, ("urls", ("fail", (..., "url")))) == expected
    ...
    def test_traversal_dict(self):
        ...
        assert traverse_obj(None, {0: "fail"}) == {}, "default to dict if pruned"
    ...
    def test_traversal_expected_type(self):
        ...
        assert (
            traverse_obj(_EXPECTED_TYPE_DATA, "str", expected_type=str) == "str"
        ...
        assert traverse_obj(_TEST_DATA, {0: 100, 1: 1.2}, expected_type=int) == {
            0: 100
        ...
        assert traverse_obj(_TEST_DATA, {0: {0: 100, 1: "str"}}, expected_type=int) == {
            0: {0: 100}
        }, "expected_type should transform deep dict values"
        assert traverse_obj(
            _TEST_DATA, [({0: "..."}, {0: "..."})], expected_type=type(...)
        ) == [{0: ...}, {0: ...}], "expected_type should transform branched dict values"
    ...
    def test_traversal_casesense(self):
        ...
        assert traverse_obj(_CASESENSE_DATA, [0, ("keY",)], casesense=False) == [
            "value1"
        ], "allow non matching key case in branch if `casesense`"
        assert traverse_obj(_CASESENSE_DATA, [0, ([0, "keY"],)], casesense=False) == [
            "value2"
    ...
    def test_traversal_traverse_string(self):
        ...
        assert (
            traverse_obj(_TRAVERSE_STRING_DATA, ("str", 0), traverse_string=True) == "s"
    ...
    def test_traversal_xml_etree(self):
        ...
        assert traverse_obj(etree, 0) == etree[0], "int key should return the nth child"
        ...
        assert traverse_obj(etree, ("country", 0, ..., "text()", {int_or_none})) == [
            1,
            2008,
            141100,
        ], "special transformations should act on current element"
    ...
    def test_traversal_unbranching(self):
        ...
        assert (
            traverse_obj(_TEST_DATA, [("dict", "None", 100, 1.2), all, {float}]) is None
        ), "`all` should reset branching status"
        assert (
            traverse_obj(_TEST_DATA, [("dict", "None", 100, 1.2), any, {float}]) is None
...
class TestTraversalHelpers:
    ...
    def test_find_element(self):
        ...
        assert (
            find_element(attr="data-id", value="y(?:es)?", regex=True)(_TEST_HTML)
            == "3"
        )
        assert (
    ...
    def test_find_elements(self):
        ...
        assert find_elements(attr="custom", value="[ez]", regex=True)(_TEST_HTML) == [
            "2",
            "3",
            "5",
        ]
