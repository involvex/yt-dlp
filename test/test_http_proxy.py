...
class HTTPConnectProxyHandler(BaseHTTPRequestHandler, HTTPProxyAuthMixin):
    ...
    def __init__(
        self, *args, username=None, password=None, request_handler=None, **kwargs
    ):
        self.username = username
        self.password = password
    ...
    def do_CONNECT(self):
        ...
        self.request_handler(
            self.request, self.client_address, self.server, proxy_info=proxy_info
        )
        self.server.close_request(self.request)
...
def proxy_server(
    ...
    try:
        ...
        server_type = (
            ThreadingTCPServer if "." in bind_address else IPv6ThreadingTCPServer
        )
        server = server_type(
            (bind_address, 0),
            functools.partial(
...
class TestHTTPProxy:
    ...
    def test_http_auth(self, handler, ctx):
        with ctx.http_server(
            ...
            with handler(
                proxies={ctx.REQUEST_PROTO: f"http://test:test@{server_address}"}
            ) as rh:
                proxy_info = ctx.proxy_info_request(rh)
                assert proxy_info["proxy"] == server_address
    ...
    def test_http_bad_auth(self, handler, ctx):
        with ctx.http_server(
            ...
            with handler(
                proxies={ctx.REQUEST_PROTO: f"http://test:bad@{server_address}"}
            ) as rh:
    ...
    def test_https(self, handler, ctx):
        with ctx.http_server(HTTPSProxyHandler) as server_address:
            with handler(
                verify=False, proxies={ctx.REQUEST_PROTO: f"https://{server_address}"}
            ) as rh:
                proxy_info = ctx.proxy_info_request(rh)
                assert proxy_info["proxy"] == server_address
                assert proxy_info["connect"] is False
    ...
    def test_https_verify_failed(self, handler, ctx):
        with ctx.http_server(HTTPSProxyHandler) as server_address:
            with handler(
                verify=True, proxies={ctx.REQUEST_PROTO: f"https://{server_address}"}
            ) as rh:
    ...
    def test_http_with_idn(self, handler, ctx):
        with ctx.http_server(HTTPProxyHandler) as server_address:
            with handler(proxies={ctx.REQUEST_PROTO: f"http://{server_address}"}) as rh:
                ...
                assert (
                    proxy_info["headers"]["Host"].split(":", 1)[0] == "xn--fiq228c.tw"
                )
...
class TestHTTPConnectProxy:
    def test_http_connect_no_auth(self, handler, ctx):
        with ctx.http_server(HTTPConnectProxyHandler) as server_address:
            with handler(
                verify=False, proxies={ctx.REQUEST_PROTO: f"http://{server_address}"}
            ) as rh:
                proxy_info = ctx.proxy_info_request(rh)
                assert proxy_info["proxy"] == server_address
    ...
    def test_https_connect_proxy_auth(self, handler, ctx):
        with ctx.http_server(
            ...
            with handler(
                verify=False,
                proxies={ctx.REQUEST_PROTO: f"https://test:test@{server_address}"},
            ) as rh:
                proxy_info = ctx.proxy_info_request(rh)
                assert proxy_info["proxy"] == server_address
                assert "Proxy-Authorization" in proxy_info["headers"]
