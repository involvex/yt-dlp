    ...
    def handle(self):
        ...
        if auth is not None and Socks5Auth.AUTH_USER_PASS not in methods:
            self.connection.sendall(
                struct.pack("!BB", SOCKS5_VERSION, Socks5Auth.AUTH_NO_ACCEPTABLE)
            )
            ...
            if username == auth[0] and password == auth[1]:
                self.connection.sendall(
                    struct.pack(
                        "!BB", SOCKS5_USER_AUTH_VERSION, SOCKS5_USER_AUTH_SUCCESS
                    )
                )
            else:
                self.connection.sendall(
                    struct.pack(
                        "!BB", SOCKS5_USER_AUTH_VERSION, SOCKS5_USER_AUTH_FAILURE
                    )
                )
                self.server.close_request(self.request)
                return
        ...
        self.request_handler_class(
            self.request, self.client_address, self.server, socks_info=socks_info
        )
