...
class TestMetadataFromField(unittest.TestCase):
    ...
    def test_format_to_regex(self):
        ...
        self.assertEqual(
            MetadataParserPP.format_to_regex(r"text (?P<x>.+)"), r"text (?P<x>.+)"
        )
        self.assertEqual(MetadataParserPP.format_to_regex("x"), r"(?s)(?P<x>.+)")
        self.assertEqual(
            MetadataParserPP.format_to_regex("Field_Name1"), r"(?s)(?P<Field_Name1>.+)"
        )
...
class TestModifyChaptersPP(unittest.TestCase):
    ...
    def _remove_marked_arrange_sponsors_test_impl(
        ...
        actual_chapters, actual_removed = self._pp._remove_marked_arrange_sponsors(
            chapters
        )
    ...
    def test_remove_marked_arrange_sponsors_CutOverlapsMultipleChapters(self):
        ...
        chapters = (
            self._chapters([20, 40, 60, 80, 100], ["c1", "c2", "c3", "c4", "c5"]) + cuts
        )
    ...
    def test_remove_marked_arrange_sponsors_TinyChaptersResultingFromSponsorOverlapAreIgnored(
        ...
        self._remove_marked_arrange_sponsors_test_impl(
            chapters,
            self._chapters([1.5, 2.5, 4], ["c1", "[SponsorBlock]: Sponsor", "c3"]),
            [],
        )
    ...
    def test_remove_marked_arrange_sponsors_TinySponsorsPrependedToTheNextSponsor(self):
        ...
        self._remove_marked_arrange_sponsors_test_impl(
            chapters,
            self._chapters([1.5, 4], ["c", "[SponsorBlock]: Unpaid/Self Promotion"]),
            [],
        )
...
"""
        ...
        self.assertEqual(
            expected, "".join(self._pp._concat_spec(["test"] * len(opts), opts))
        )
