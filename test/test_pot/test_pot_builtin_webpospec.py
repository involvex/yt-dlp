...
def pot_request(pot_request) -> PoTokenRequest:
    pot_request.visitor_data = (
        "CgsxMjNhYmNYWVpfLSiA4s%2DqBg%3D%3D"  # visitor_id=123abcXYZ_-
    )
    return pot_request
...
class TestWebPoPCSP:
    ...
    @pytest.mark.parametrize(
        "client_name, context, is_authenticated",
        [
    ...
    def test_not_supports(
        self, ie, logger, pot_request, client_name, context, is_authenticated
    ):
        pcs = WebPoPCSP(ie=ie, logger=logger, settings={})
    ...
    @pytest.mark.parametrize(
        "client_name, context, is_authenticated, remote_host, source_address, request_proxy, expected",
        [
            *[
                (
                    client,
                ...
                for context, is_authenticated, remote_host, source_address, request_proxy, expected in [
                    (
                        ...
                        {
                            "t": "webpo",
                            "ip": "example-remote-host",
                            "sa": "example-source-address",
                            "px": "example-request-proxy",
                            "cb": "123abcXYZ_-",
                            "cbt": "visitor_id",
                        },
                    ),
    ...
    def test_generate_key_bindings(
        self,
        ie,
        logger,
        pot_request,
    ...
    def test_no_bind_visitor_id(self, ie, logger, pot_request):
        ...
        pcs = WebPoPCSP(
            ie=ie, logger=logger, settings={"bind_to_visitor_id": ["false"]}
        )
        ...
        assert pcs.generate_cache_spec(pot_request).key_bindings == {
            "t": "webpo",
            "ip": None,
            "sa": None,
            "px": None,
            "cb": "CgsxMjNhYmNYWVpfLSiA4s%2DqBg%3D%3D",
            "cbt": "visitor_data",
        }
    ...
    def test_default_ttl(self, ie, logger, pot_request):
        pcs = WebPoPCSP(ie=ie, logger=logger, settings={})
        assert (
            pcs.generate_cache_spec(pot_request).default_ttl == 6 * 60 * 60
        )  # should default to 6 hours

    def test_write_policy(self, ie, logger, pot_request):
        pcs = WebPoPCSP(ie=ie, logger=logger, settings={})
        pot_request.context = PoTokenContext.GVS
        assert (
            pcs.generate_cache_spec(pot_request).write_policy
