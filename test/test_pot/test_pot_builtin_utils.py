...
class TestGetWebPoContentBinding:
    ...
    @pytest.mark.parametrize(
        "client_name, context, is_authenticated, expected",
        [
            *[
                (client, context, is_authenticated, expected)
                for client in [
                    "WEB",
                    "MWEB",
                    "TVHTML5",
                ...
                for context, is_authenticated, expected in [
                    (
                        PoTokenContext.GVS,
                        False,
                        ("example-visitor-data", ContentBindingType.VISITOR_DATA),
                    ),
                    (
                        PoTokenContext.PLAYER,
                        False,
                        ("example-video-id", ContentBindingType.VIDEO_ID),
                    ),
                    (
                        PoTokenContext.SUBS,
                        False,
                    ...
                    (
                        PoTokenContext.GVS,
                        True,
                        ("example-data-sync-id", ContentBindingType.DATASYNC_ID),
                    ),
                ]
            ],
            (
    ...
    def test_get_webpo_content_binding(
        self, pot_request, client_name, context, is_authenticated, expected
    ):
    ...
    def test_extract_visitor_id(self, pot_request):
        ...
        assert get_webpo_content_binding(pot_request, bind_to_visitor_id=True) == (
            "123abcXYZ_-",
            ContentBindingType.VISITOR_ID,
        )
    ...
    def test_invalid_visitor_id(self, pot_request):
        ...
        assert get_webpo_content_binding(pot_request, bind_to_visitor_id=True) == (
            ...
            ContentBindingType.VISITOR_DATA,
        )
    ...
    def test_gvs_video_id_binding_experiment(self, pot_request):
        ...
        assert get_webpo_content_binding(pot_request) == (
            "example-video-id",
            ContentBindingType.VIDEO_ID,
        )
