...
class TestMemoryLRUPCS:
    ...
    def pcp(self, ie, logger) -> MemoryLRUPCP:
        return MemoryLRUPCP(
            ...
            logger,
            {},
            initialize_cache=lambda max_size: (
                OrderedDict(),
                threading.Lock(),
                max_size,
            ),
    ...
    def test_use_global_cache_default(self, ie, logger):
        ...
        assert (
            pcp.max_size == pcp2.max_size == _pot_memory_cache.value["max_size"] == 25
        )
    ...
    def test_fail_max_size_change_global(self, ie, logger):
        ...
        with pytest.raises(
            ValueError,
            match="Cannot change max_size of initialized global memory cache",
        ):
