...
class TestPoTokenProvider:
    ...
    def test_example_provider_success(self, ie, logger, pot_request):
        ...
        assert (
            provider.BUG_REPORT_MESSAGE
            == "please report this issue to the provider developer at  https://example.com/issues  ."
        )
    ...
    def test_provider_unsupported_external_request_source_address(
        self, ie, logger, pot_request
    ):
        class InternalPTP(ExamplePTP):
    ...
    def test_provider_supported_external_request_source_address(
        self, ie, logger, pot_request
    ):
        class InternalPTP(ExamplePTP):
            _SUPPORTED_EXTERNAL_REQUEST_FEATURES = (
    ...
    def test_provider_unsupported_external_request_tls_verification(
        self, ie, logger, pot_request
    ):
        class InternalPTP(ExamplePTP):
            _SUPPORTED_EXTERNAL_REQUEST_FEATURES = tuple()
    ...
    def test_provider_supported_external_request_tls_verification(
        self, ie, logger, pot_request
    ):
        class InternalPTP(ExamplePTP):
            _SUPPORTED_EXTERNAL_REQUEST_FEATURES = (
    ...
    def test_provider_request_webpage(self, ie, logger, pot_request):
        ...
        pot_request.request_headers = HTTPHeaderDict(
        ...
        sent_request = provider._request_webpage(
            Request(
                "https://example.com",
            ),
    ...
    def test_provider_request_webpage_override(self, ie, logger, pot_request):
        ...
        sent_request = provider._request_webpage(
            ...
            pot_request=pot_request,
            note="Custom requesting webpage",
        )
    ...
    def test_get_config_arg(self, ie, logger):
        provider = ExamplePTP(
            ie=ie, logger=logger, settings={"abc": ["123D"], "xyz": ["456a", "789B"]}
        )
...
class TestPoTokenCacheProvider:
    ...
    def test_get_config_arg(self, ie, logger):
        provider = ExampleCacheProviderPCP(
            ie=ie, logger=logger, settings={"abc": ["123D"], "xyz": ["456a", "789B"]}
        )
        assert provider._configuration_arg("abc") == ["123d"]
        assert provider._configuration_arg("abc", default=["default"]) == ["123d"]
...
class TestPoTokenCacheSpecProvider:
    ...
    def test_create_provider_barebones(self, ie, pot_request, logger):
        ...
        assert (
            provider.generate_cache_spec(request=pot_request).write_policy
    ...
    def test_create_provider_example(self, ie, pot_request, logger):
        ...
        assert provider.generate_cache_spec(pot_request).key_bindings == {
            "field": "example-key"
        }
        assert provider.generate_cache_spec(pot_request).default_ttl == 60
        assert (
            provider.generate_cache_spec(pot_request).write_policy
            == CacheProviderWritePolicy.WRITE_FIRST
        )

    def test_get_config_arg(self, ie, logger):
        provider = ExampleCacheSpecProviderPCSP(
            ie=ie, logger=logger, settings={"abc": ["123D"], "xyz": ["456a", "789B"]}
        )
...
class TestPoTokenRequest:
    def test_copy_request(self, pot_request):
        ...
        assert (
            copied_request.request_source_address == pot_request.request_source_address
        )
...
def test_provider_bug_report_message(ie, logger):
    ...
    assert (
        message_before
        == "custom message! Please report this issue to the provider developer at  https://example.com/issues  ."
    )
...
def test_register_cache_provider(ie):
    ...
    assert (
        _pot_cache_providers.value.get("UnavailableCacheProvider")
...
def test_register_cache_provider_spec(ie):
    ...
    assert (
        _pot_pcs_providers.value.get("UnavailableCacheProvider")
        == UnavailableCacheProviderPCSP
    )
