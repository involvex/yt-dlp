...
class TestUtil(unittest.TestCase):
    ...
    def test_sanitize_path(self):
        ...
        self.assertEqual(sanitize_path("abc.../def..."), "abc..#\\def..#")
        self.assertEqual(
            sanitize_path("C:\\abc:%(title)s.%(ext)s"), "C:\\abc#%(title)s.%(ext)s"
        )
        ...
        for test, expected in [
            ...
            assert result == ntpath.normpath(
                test
            ), f"{test} does not match ntpath.normpath"
    ...
    def test_datetime_round(self):
        self.assertEqual(
            datetime_round(
                dt.datetime.strptime("1820-05-12T01:23:45Z", "%Y-%m-%dT%H:%M:%SZ")
            ),
            dt.datetime(1820, 5, 12, tzinfo=dt.timezone.utc),
        )
        self.assertEqual(
            datetime_round(
                dt.datetime.strptime("1969-12-31T23:34:45Z", "%Y-%m-%dT%H:%M:%SZ"),
                "hour",
            ),
        ...
        self.assertEqual(
            datetime_round(
                dt.datetime.strptime("2024-12-25T01:23:45Z", "%Y-%m-%dT%H:%M:%SZ"),
                "minute",
            ),
        ...
        self.assertEqual(
            datetime_round(
                dt.datetime.strptime(
                    "2024-12-25T01:23:45.123Z", "%Y-%m-%dT%H:%M:%S.%fZ"
                ),
                "second",
            ),
        ...
        self.assertEqual(
            datetime_round(
                dt.datetime.strptime(
                    "2024-12-25T01:23:45.678Z", "%Y-%m-%dT%H:%M:%S.%fZ"
                ),
                "second",
            ),
    ...
    def test_unified_timestamps(self):
        ...
        self.assertEqual(
            unified_timestamp("2026-01-01 00:00:00", tz_offset=0), 1767225600
        )
        self.assertEqual(
            unified_timestamp("2026-01-01 00:00:00", tz_offset=8), 1767196800
        )
        self.assertEqual(
            unified_timestamp("2026-01-01 00:00:00 +0800", tz_offset=-5), 1767196800
        )
    ...
    def test_parse_iso8601(self):
        ...
        self.assertEqual(
            parse_iso8601("2014-03-23T23:04:26", timezone=dt.timedelta(hours=-7)),
            1395641066,
        )
        self.assertEqual(
...
"""
    ...
    def test_get_elements_text_and_html_by_attribute(self):
        ...
        self.assertEqual(
            ...
            list(
                zip(["nice", "also nice"], self.GET_ELEMENTS_BY_CLASS_RES, strict=True)
            ),
        )
        self.assertEqual(
    ...
    def test_http_header_dict(self):
        ...
        self.assertEqual(
            set(headers.sensitive().items()), {("Ytdl-test", "2"), ("X-dlp", "data")}
        )
        ...
        self.assertEqual(
            set(headers2.sensitive().items()), {("Ytdl-test", "2"), ("X-dlP", "data2")}
        )
    ...
    def test_jwt_encode(self):
        def test(expected, headers={}):
            self.assertEqual(
                jwt_encode(self._JWT_DECODED, self._JWT_KEY, headers=headers), expected
            )
        ...
        test(
            self._JWT_WITH_REORDERED_HEADERS_AND_RS256_ALG, headers=self._JWT_HEADERS_3
        )
