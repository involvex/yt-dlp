...
class TestWebsSocketRequestHandlerConformance:
    ...
    def test_legacy_ssl_extension(self, handler):
        with handler(verify=False) as rh:
            ws = ws_validate_and_send(
                rh, Request(self.legacy_wss_host, extensions={"legacy_ssl": True})
            )
            assert ws.status == 101
            ws.close()
    ...
    def test_cookies(self, handler):
        ...
        with handler() as rh:
            ...
            ws = ws_validate_and_send(
                rh, Request(self.ws_base_url, extensions={"cookiejar": cookiejar})
            )
            ws.send("headers")
            assert HTTPHeaderDict(json.loads(ws.recv()))["cookie"] == "test=ytdlp"
            ws.close()
    ...
    def test_cookie_sync_only_cookiejar(self, handler):
        ...
        with handler() as rh:
            ...
            ws = ws_validate_and_send(
                rh,
                Request(
                    self.ws_base_url, extensions={"cookiejar": YoutubeDLCookieJar()}
                ),
            )
            ws.send("headers")
            assert "cookie" not in HTTPHeaderDict(json.loads(ws.recv()))
...
class TestWebsocketsRequestHandler:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                lambda: websockets.exceptions.InvalidURI(msg="test", uri="test://"),
                RequestError,
            ),
