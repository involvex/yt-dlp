...
from devscripts.setup_variables import (
    STABLE_REPOSITORY,
    process_inputs,
    setup_variables,
)
...
def _test(
    ...
    if expected is None:
        assert (
            result is None
        ), f"expected error/None but got dict: {github_repository} {note}"
        return

    exp = expected.copy()
    if ignore_revision:
        assert len(result["version"]) == len(
            exp["version"]
        ), f"revision missing: {github_repository} {note}"
...
def test_setup_variables():
    DEFAULT_VERSION_WITH_REVISION = dt.datetime.now(tz=dt.timezone.utc).strftime(
        "%Y.%m.%d.%H%M%S"
    )
    ...
    _test(
        STABLE_REPOSITORY,
        "official vars/secrets, nightly (w/o target)",
        BASE_REPO_VARS,
        BASE_REPO_SECRETS,
        {
            "source": "nightly",
    ...
    _test(
        STABLE_REPOSITORY,
        "official vars/secrets, master (w/o target)",
        BASE_REPO_VARS,
        BASE_REPO_SECRETS,
        {
            "source": "master",
    ...
    _test(
        STABLE_REPOSITORY,
        "official vars/secrets, special tag, updates to stable",
        BASE_REPO_VARS,
        BASE_REPO_SECRETS,
        {
    ...
    _test(
        STABLE_REPOSITORY,
        'official vars/secrets, special tag, "stable" as target repo',
        BASE_REPO_VARS,
        BASE_REPO_SECRETS,
        {
    ...
    _test(
        FORK_REPOSITORY,
        "fork w/o vars/secrets, prerelease",
        {},
        {},
        {"prerelease": True},
        {
    ...
    _test(
        FORK_REPOSITORY,
        "fork w/o vars/secrets, revision",
        {},
        {},
        {"version": "123"},
        {
    ...
    _test(
        FORK_REPOSITORY,
        "fork, non-numeric tag, updates to stable",
        {},
        {},
        {
            "prerelease": True,
    ...
    _test(
        STABLE_REPOSITORY,
        "official vars but no ARCHIVE_REPO_TOKEN, nightly",
        BASE_REPO_VARS,
        {},
        {
            "source": "nightly",
