...
def fetch_github_tags(
    ...
    *,
    ...
    for page in itertools.count(1):
        print(
            f"Fetching tags list page {page} from Github API: {owner}/{repo}",
            file=sys.stderr,
        )
        tags = call_github_api(
            f"/repos/{owner}/{repo}/tags", query={"per_page": "100", "page": page}
        )
    ...
    for name, value in table.items():
        ...
        for element in value:
            ...
            if isinstance(element, dict):
                yield "{ " + ", ".join(
                    f"{k} = {json.dumps(v)}" for k, v in element.items()
                ) + " }"
            else:
                yield f'"{element}"'
            yield ",\n"
...
def run_uv_export(
    *,
    ...
    return run_process(
        ...
        *(
            [f"--output-file={output_file.relative_to(BASE_PATH)}"]
            if output_file
            else []
        ),
    ).stdout
...
def makefile_variables(
    ...
    *,
    ...
    if not all(
        arg is not None for arg in (version, name, digest, not filetypes or data)
    ):
        raise ValueError(
...
def update_ejs(verify: bool = False) -> dict[str, tuple[str | None, str | None]] | None:
    ...
    print(
        f"Updating {PACKAGE_NAME} from {current_version} to {version}", file=sys.stderr
    )
    hashes = []
    wheel_info = {}
    for asset in info["assets"]:
        ...
        if hexdigest != expected:
            raise ValueError(
                f"downloaded attest mismatch ({hexdigest!r} != {expected!r})"
            )

        if is_wheel:
            wheel_info = ejs_makefile_variables(
                version=version, name=name, digest=digest, data=data
            )
            continue
    ...
    if missing_fields := [key for key in makefile_info if not wheel_info.get(key)]:
        raise ValueError(
            f'wheel info not found in release: {", ".join(missing_fields)}'
        )
    ...
    for key in wheel_info:
        makefile = makefile.replace(
            f"{key} = {makefile_info[key]}", f"{key} = {wheel_info[key]}"
        )
...
def parse_version_from_dist(
    filename: str, name: str, *, require: bool = False
) -> str | None:
    ...
    if require:
        raise ValueError(
            f"unable to parse version from distribution filename: {filename}"
        )
...
def parse_dependency(line: str, *, require_version: bool = False) -> Dependency:
    ...
    specifier, _, direct_reference = map(
        str.strip, specifier_or_direct_reference.partition("@")
    )
    ...
    if require_version and not exact_version:
        raise ValueError(
            f"unable to parse Dependency.exact_version from line:\n    {line}"
        )
...
def update_requirements(
    ...
    if not upgrade_only or upgrade_only.lower() == "pyinstaller":
        ...
        for target_suffix, asset_tag in PYINSTALLER_BUILDS_TARGETS.items():
            asset_info = next(
                asset for asset in info["assets"] if asset_tag in asset["name"]
            )
            ...
            requirements_path = REQUIREMENTS_PATH / REQS_OUTPUT_TMPL.format(
                target_suffix
            )
            ...
            new_requirements_txt = PYINSTALLER_BUILDS_TMPL.format(
                pyinstaller_builds_deps,
                asset_info["browser_download_url"],
                asset_info["digest"],
            )
            requirements_path.write_text(new_requirements_txt)
            all_updates.update(
                evaluate_requirements_txt(old_requirements_txt, new_requirements_txt)
            )
...
def generate_report(
    ...
    for package, (old, new) in sorted(all_updates.items()):
        if package in WELLKNOWN_PACKAGES:
            ...
            if changelog:
                name = (
                    urllib.parse.urlparse(changelog).path.rstrip("/").rpartition("/")[2]
                    or "changelog"
                )
        ...
        if old and new:
            ...
            offset = None
            for index, (old_part, new_part) in enumerate(
                zip(old_parts, new_parts, strict=False)
            ):
            ...
            if offset is not None:
                md_old = (
                    ".".join(old_parts[:offset])
                    + ".***"
                    + ".".join(old_parts[offset:])
                    + "***"
                )
                md_new = (
                    ".".join(new_parts[:offset])
                    + ".***"
                    + ".".join(new_parts[offset:])
                    + "***"
                )
...
def make_commit_body(all_updates: dict[str, tuple[str | None, str | None]]) -> str:
    return "\n".join(
        make_commit_line(package, old, new)
...
def table_a_raza(
    ...
    yield " | ".join(
        col.ljust(width) for width, col in zip(widths, header, strict=True)
    )
    yield "-|-".join("".ljust(width, "-") for width in widths)
    for row in rows:
        yield " | ".join(
            col.ljust(width) for width, col in zip(widths, row, strict=True)
        )
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="generate/update lockfile and requirements"
    )
    parser.add_argument(
        "upgrade_only",
        nargs="?",
...
def main():
    ...
    if args.upgrade_only in ("ejs", "yt-dlp-ejs"):
        ...
        all_updates = update_requirements(
            upgrade_only=args.upgrade_only, verify=args.verify
        )
