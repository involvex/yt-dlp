...
class Changelog:
    MISC_RE = re.compile(
        r"(?:^|\b)(?:lint(?:ing)?|misc|format(?:ting)?|fixes)(?:\b|$)", re.IGNORECASE
    )
    ...
    def format_single_change(self, info: CommitInfo):
        ...
        if info.fixes:
            fix_message = ", ".join(
                f"{self._format_message_link(None, fix.hash)}" for fix in info.fixes
            )
...
class CommitRange:
        ...
        for i, commit_hash in enumerate(lines):
            ...
            for line in iter(lambda: next(lines), self.COMMIT_SEPARATOR):
                if match := self.AUTHOR_INDICATOR_RE.match(line):
                    authors = sorted(
                        map(str.strip, line[match.end() :].split(",")), key=str.casefold
                    )
        ...
        for override in overrides:
            ...
            if override["action"] == "add":
                ...
                if override_hash not in self._commits:
                    continue
                commit = Commit(
                    override_hash, override["short"], override.get("authors") or []
                )
                ...
                if match := self.FIXES_RE.search(commit.short):
                    ...
                    if fix_commitish in self._commits:
                        ...
                        logger.info(
                            f"Found fix for {fix_commitish[:HASH_LENGTH]}: {commit.hash[:HASH_LENGTH]}"
                        )
                        continue
    ...
    def groups(self):
        ...
        for commit in self:
            ...
            issues = (
                [issue.strip()[1:] for issue in issues.split(",")] if issues else []
            )
